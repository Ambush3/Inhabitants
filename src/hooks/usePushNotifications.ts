...
export function usePushNotifications(
    ...
    useEffect(() => {
        ...
        const subscription = Notifications.addNotificationResponseReceivedListener(async (response) => {
            console.log('listener fired, hasHandledTap:', hasHandledTap);
            if (hasHandledTap) return;
            hasHandledTap = true;
            setTimeout(() => { hasHandledTap = false; }, 2000);
            ...
            const lat = params.get('deepLinkLat');
            const lng = params.get('deepLinkLng');
            console.log('parsed params:', { spot_id, lat, lng });

            if (sessionRef.current) {
                console.log('navigating via onSpotOpen');
                if (spot_id && lat && lng) {
    ...
    async function registerToken(userId: string) {
        ...
        const { error } = await supabase
            .from('push_tokens')
            .upsert({ user_id: userId, token: tokenData.data, updated_at: new Date().toISOString() }, { onConflict: 'user_id' });
