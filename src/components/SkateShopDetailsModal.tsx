...
export function SkateShopDetailsModal({
    ...
    const { override, isVetted, loadOverride, saveOverride, resetOverride } =
        usePlaceOverrides();
    ...
    const phone =
        override?.phone ?? tags['phone'] ?? tags['contact:phone'] ?? null;
    const website =
        override?.website ?? tags['website'] ?? tags['contact:website'] ?? null;
            ...
            async (buttonIndex) => {
                if (buttonIndex === 1) {
                    ...
                    await Linking.openURL(
                        `maps://app?q=${encodedName}&ll=${place.lat},${place.lng}`
                    );
